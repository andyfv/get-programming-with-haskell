...
bookToHtml book = mconcat 
    ...
    where titleInTags = mconcat ["<strong>", (title book), "</string>\n"]
...
splitDirectory directory = 
    ...
    where (nextEntry, restEntries) = B.splitAt dirEntryLenght directory
...
getTextField record fieldMetadata = E.decodeUtf8 byteStringValue
    where recordLength = getRecordLength record
          ...
          byteStringValue = B.take (fieldLenght fieldMetadata) baseAtEntry
...
pairsToBooks pairs = map toBook justPairs
    where justPairs = 
                filter (\(title, author) -> isJust title && isJust author) pairs
          toBook =  
                \(title, author) -> Book { title = fromJust title
